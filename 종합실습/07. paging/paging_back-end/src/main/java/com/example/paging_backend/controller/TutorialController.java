package com.example.paging_backend.controller;

import com.example.paging_backend.model.Tutorial;
import com.example.paging_backend.paging.Criteria;
import com.example.paging_backend.service.TutorialServiceImp;
...
//            if (bSuccess == true) 밑에 조건과 동일함
    ...
    public ResponseEntity<Map<String, Object>> getAllTitlePage(Criteria criteria) {
        ...
        // *title(제목)을 조회하는 서비스를 호출, totalItem과 totalPages 계산 기능 포함(현재 null)
        List<Tutorial> tutorials = tutorialService.findByTitleContaining(criteria);
        // *findByTitleContaining이 호출되면서 totalItem과 totalPages 값도 같이 나옴
