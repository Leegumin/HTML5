...
//        !return queryResult == 1 ? true : false; -> 단순화시킨 코드가 아래 코드
    ...
    public CommentDto getCommentDetail(Long id) {
        ...
        return commentDao.selectCommentDetail(id);
    }
    ...
    public boolean deleteComment(Long id) {
        ...
        CommentDto comment = commentDao.selectCommentDetail(id);

        // *Delete_Yn : "N" 일때만 AND Comment !=null 아닐때 삭제 진행
        if (comment != null && "N".equals(comment.getDeleteFlag())) {
            queryResult = commentDao.deleteComment(id);
        }
        return queryResult == 1;
    }
}
