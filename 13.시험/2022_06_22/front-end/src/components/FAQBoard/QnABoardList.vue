<template>
  <div class = "container mt-3">
      ...
      <div class = "input-group mb-3">
        ...
        <div class = "input-group-append">
          <button class = "btn btn-outline-dark"
                  type = "button"
                  @click = "page =1; retrieveQnABoards();"
          >
            Search
          </button>
        </div>
    ...
    <p class="text-center h3">QnA List</p>
    <table class = "table">
      <thead>
      <tr>
        <th scope = "col">No</th>
      ...
      <!--#조회 데이터 생 부분-->
      <!--Todo: :class, @click 추가-->
      <tbody v-for = "(qnaBoard, index) in qnaBoards"
             :key = "index"
             :class = "{ active: index == currentIndex}"
             @click = "setActiveQnABoard(qnaBoard,index)"
      >
      <tr>
        <td>{{ qnaBoard.id }}</td>
        ...
        <!--* http://localhost8080/qnaBoards/qnaBoard.id 형태의 주소로 이동-->
        <!--* :href -> v-bind:href, qnaBoard.id 값을 가져오기 위해서는 <script>의 data()와 바인딩할 필요가 있음-->
        <td><a :href = "'/qna-boards/' + qnaBoard.id"
...
export default {
  ...
  methods: {
    ...
    retrieveQnABoards () {
      ...
      QnABoardDataService.getAll(params)
          // 성공하면 then으로 들어옴
          // *response : 헤더(상태정보, 쿠키 등), 바디(json) 등을 포함하고 있음
          // *data : back-end(스프링)에서 보내준 qnaBoard 객체가 담김
          ...
          .then(response => {
            ...
            // 임시변수 qnaBoards, totalItems( 서버의 결과 데이터가 들어옴 )
            const {
                    qnaBoards,
                    totalItems,
                  } = response.data
            this.qnaBoards = qnaBoards // 객체
    ...
    // 새로고침 메소드
    refreshList () {
      this.currentQnABoard = null
      this.currentIndex = -1
      this.retrieveQnABoards()
    },
    // vue의 data 변수에 값을 저장하는 메소드(클릭했을 때 현재값 저장)
    setActiveQnABoard (qnaBoard, index) {
      this.currentQnABoard = qnaBoard
